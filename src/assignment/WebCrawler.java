    ...
    public static void main(String[] args) {
        ...
        try {
            while (!remaining.isEmpty()) {
                //set current URL in handler
                handler.setCurrentURL(remaining.peek());
                // Parse the next URL's page
                URL urltoOpen = remaining.poll();
                try {
                    parser.parse(new InputStreamReader(Objects.requireNonNull(urltoOpen).openStream()), handler);
                } catch (Exception e) {
                    System.err.println("BAD URL " + urltoOpen);
                }
                // Add any new URLs
